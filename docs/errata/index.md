...
- 第 10 页，“指针运算符”这个名字比较少见；其官方名称为“间接引用运算符”，我称之为“解地址运算符”。
- 第 20 页直到最后，以及本课件之后的所有课件，都建议用 `nullptr` 而不是 `NULL`。
- 第 26 页，“实型元素”应为“`float` 类型元素”。本页中的所有二元加法、乘法，均按照单字节的运算进行。整型元素、`float` 类型元素的事迹大小由实现决定。
- 第 32 页，应为“`a` 被转换为数组首元素的地址”。
- 第 38 页，“`a` 与 `&a[0]` 等价”句不合适，应改为“`a` 相当于 `&a[0]`”或“`a` 代表 `&a[0]`”或“`a` 转换到 `&a[0]`”。
- 第 39 页，`a++` 没有意义是因为，`a` 作为数组不能 `++`；转换到指针后虽然可以 `+1`，但不能被赋值，所以不能 `++`。
- 第 49、55 页，“下沉”“上浮”的说法有些无稽之谈，删去。
- 第 50 页，前两个“等价”替换为“相当”。
- 第 59-61、63-65 页，`void main()` 改为 `int main()`。
- 第 68 页，“VC 中 4 字节”改为“32 位操作系统下 4 字节”。
